...
import { Link as LinkIcon, Type, Wifi, Contact, Phone, Mail, FileDown, Loader2, Image as ImageIcon, Video as VideoIcon, UploadCloud, MapPin, CreditCard, MessageCircle, MessageSquare, FileCode2 } from 'lucide-react';
...
const QRGenerator = () => {
  ...
  const [codeFile, setCodeFile] = useState<{name: string, size: number, ext: string} | null>(null);
  ...
  const tabs = [
    ...
    { id: 'code', label: 'Code', icon: <FileCode2 size={16} /> },
  ...
  const handleMakeDynamic = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('qrcodes').insert({
        ...
        original_url: activeTab === 'url' ? qrValue : null,
        type: activeTab.toUpperCase(),
        data: activeTab !== 'url' ? { raw: qrValue } : null,
        scans: 0,
        analytics: [],
        password: dynamicPassword.trim() || null,
        expires_at: dynamicExpiresAt ? new Date(dynamicExpiresAt).toISOString() : null,
        max_scans: dynamicMaxScans ? parseInt(dynamicMaxScans, 10) : null,
        is_locked: false
      });
      
      if (error) throw error;
      
      setQrValue(`${window.location.origin}${window.location.pathname}#/${short_id}`);
            ...
            {activeTab === 'code' && (
              <div className="border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center bg-muted/20 relative group overflow-hidden">
                  ...
                  onChange={(e)=>{
                    ...
                    reader.onload = (event) => {
                      if (event.target?.result) {
                        setQrValue(event.target.result as string);
                        setCodeFile({ name: file.name, size: file.size, ext: ext.toUpperCase() });
                      }
                    };
                    reader.onerror = () => {
                ...
                <div className="flex flex-col items-center pointer-events-none">
                  {codeFile ? (
                    <>
                      <FileCode2 className="w-10 h-10 text-primary mb-3" />
                      <span className="text-sm font-semibold">{codeFile.name}</span>
                      <span className="text-xs text-muted-foreground mt-1 bg-background px-2 py-0.5 rounded-full border shadow-sm">
                        {codeFile.ext} • {(codeFile.size / 1024).toFixed(1)} KB
                      </span>
                      <span className="text-xs text-primary mt-3 group-hover:underline">Click or drag to replace</span>
                    </>
                  ) : (
                    <>
