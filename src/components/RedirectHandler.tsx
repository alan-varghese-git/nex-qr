...
const RedirectHandler = () => {
  ...
  useEffect(() => {
    const handleScan = async () => {
      try {
        if (!shortId) return;
        const { data: qrData, error: fetchError } = await supabase.from('qrcodes').select('*').eq('short_id', shortId).single();
        ...
        if (qrData.max_scans && (qrData.scans || 0) >= qrData.max_scans) {
        ...
        const newAnalytics = [...(qrData.analytics || []), {
        ...
        }).eq('short_id', shortId).then(({ error }) => {
  ...
  const executeRedirect = (qrData: any) => {
    if (qrData.type === 'URL' && qrData.original_url) {
      window.location.replace(qrData.original_url);
