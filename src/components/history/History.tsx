...
const History = () => {
  ...
  const handleDelete = async (e: React.MouseEvent, shortId: string) => {
    ...
    try {
      const { error } = await supabase.from('qrcodes').delete().eq('short_id', shortId);
      if(error) throw error;
      setHistory(history.filter(item => item.short_id !== shortId));
    } catch (err) {
  ...
  const filteredHistory = history.filter(item => 
    ...
    (item.original_url && item.original_url.toLowerCase().includes(searchQuery.toLowerCase())) ||
    item.short_id.toLowerCase().includes(searchQuery.toLowerCase())
  );
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y">
                {filteredHistory.map((item) => (
                  <React.Fragment key={item.short_id}>
                    <tr 
                      className={`hover:bg-muted/30 transition-colors cursor-pointer ${expandedId === item.short_id ? 'bg-muted/10' : ''}`}
                      onClick={() => setExpandedId(expandedId === item.short_id ? null : item.short_id)}
                    >
                      ...
                      <td className="px-6 py-4 font-mono text-xs">{item.short_id}</td>
                      <td className="px-6 py-4 truncate max-w-[200px]" title={item.original_url || JSON.stringify(item.data)}>
                        {item.original_url || 'Data Payload'}
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2 items-center">
                          <a 
                            href={`${window.location.origin}/${item.short_id}`}
                            target="_blank"
                            rel="noreferrer"
                          ...
                          <button 
                            onClick={(e) => handleDelete(e, item.short_id)}
                            className="p-2 text-muted-foreground hover:text-destructive transition-colors"
                          >
                            <Trash2 size={18} />
                          </button>
                          <div className="p-2 text-muted-foreground">
                            {expandedId === item.short_id ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
                          </div>
                        </div>
                      </td>
                    </tr>
