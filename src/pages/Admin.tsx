...
const Admin = () => {
  ...
  const fetchGlobalQRs = async () => {
    try {
      ...
      setQrcodes(data as GlobalQRCode[]);
    } catch (err) {
  ...
  const handleToggleLock = async (qr: GlobalQRCode) => {
    try {
      const { error } = await supabase.from('qrcodes').update({ is_locked: !qr.is_locked }).eq('short_id', qr.short_id);
      if (error) throw error;
      setQrcodes(prev => prev.map(q => q.short_id === qr.short_id ? { ...q, is_locked: !q.is_locked } : q));
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const filteredQRs = qrcodes.filter(q => 
    q.short_id.toLowerCase().includes(searchTerm.toLowerCase()) ||
    (q.original_url && q.original_url.toLowerCase().includes(searchTerm.toLowerCase())) ||
            ...
            <tbody className="divide-y">
              {filteredQRs.map(qr => (
                <tr key={qr.short_id} className="hover:bg-muted/30 transition-colors">
                  <td className="px-6 py-4 font-mono font-medium">{qr.short_id}</td>
                  <td className="px-6 py-4">
                    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-primary/10 text-primary">
                      {qr.type}
                    </span>
                  </td>
                  <td className="px-6 py-4 max-w-[200px] truncate">{qr.original_url || 'Raw Payload'}</td>
                  ...
                  <td className="px-6 py-4 text-right space-x-2">
                    <button
                      ...
                      className={`p-2 rounded-lg border transition-colors ${qr.is_locked ? 'bg-amber-500/10 text-amber-600 border-amber-500/30' : 'hover:bg-muted text-muted-foreground'}`}
                      title={qr.is_locked ? "Unlock QR" : "Lock/Disable QR"}
                    >
                      {qr.is_locked ? <Lock size={16} /> : <Unlock size={16} />}
                    </button>
                    <button
